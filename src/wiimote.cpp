...
template<class T> inline T square(const T& val)
{
    return val * val;
}
...
{ _T("Left"), _T("Right"), _T("Down"), _T("Up"), _T("Plus"), _T("??"), _T("??"),
        _T("??"), _T("Two"), _T("One"), _T("B"), _T("A"), _T("Minus"),
        _T("??"), _T(
                "??"), _T("Home") };

const TCHAR* wiimote::ClassicButtonNameFromBit[TOTAL_BUTTON_BITS] =
{ _T("??"), _T("TrigR"), _T("Plus"), _T("Home"), _T("Minus"), _T("TrigL"), _T(
        "Down"), _T("Right"), _T("Up"), _T("Left"), _T("ZR"), _T("X"), _T("A"), _T(
                "Y"), _T("B"), _T("ZL") };
// ------------------------------------------------------------------------------------
wiimote::wiimote() :
            DataRead(CreateEvent(NULL, FALSE, FALSE, NULL)), Handle(
                    INVALID_HANDLE_VALUE), ReportType(IN_BUTTONS), bStatusReceived(
                            false), // for output method detection
                            bConnectInProgress(true), bInitInProgress(false), bEnablingMotionPlus(
                                    false), bConnectionLost(false), // set if write fails after connection
                                    bMotionPlusDetected(false), bMotionPlusEnabled(false), bMotionPlusExtension(
                                            false), bCalibrateAtRest(false), bUseHIDwrite(false), // if OS supports it
                                            ChangedCallback(NULL), CallbackTriggerFlags(CHANGED_ALL), InternalChanged(
                                                    NO_CHANGE), CurrentSample(NULL), HIDwriteThread(NULL), ReadParseThread(
                                                            NULL), SampleThread(NULL), AsyncRumbleThread(NULL), AsyncRumbleTimeout(
                                                                    0), UniqueID(0)   // not _guaranteed_ unique, see comments in header
...
#endif
{
    ...
    if (++_TotalCreated == 1)
    {
        ...
        {
            _HidD_SetOutputReport = (hidwrite_ptr) GetProcAddress(HidDLL,
...
bool wiimote::Connect(unsigned wiimote_index, bool force_hidwrites)
{
    ...
    HDEVINFO dev_info = SetupDiGetClassDevs(&guid, NULL, NULL,
    ...
    while (SetupDiEnumDeviceInterfaces(dev_info, NULL, &guid, index, &didata))
    {
        ...
        DWORD req_size = 0;
        SetupDiGetDeviceInterfaceDetail(dev_info, &didata, NULL, 0, &req_size,
                NULL);
        ...
        Handle = CreateFile(didetail->DevicePath, 0,
        ...
        if (HidD_GetAttributes(Handle, &attrib))
        {
            ...
            Handle = CreateFile(didetail->DevicePath, GENERIC_READ | GENERIC_WRITE,
                    FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED,
                    NULL);
            if (Handle == INVALID_HANDLE_VALUE)
            {
            ...
            else
            {
                ...
                TRACE(_T(".. WriteFile() %s."),
                        bStatusReceived ? _T("succeeded") : _T("failed"));
            }
...
unsigned __stdcall wiimote::ReadParseThreadfunc(void* param)
{
    ...
    while (1)
    {
        ...
        {
            connection_lost: TRACE(_T(
                    "read thread: connection to wiimote was lost"));
            remote.Disconnect();
            remote.InternalChanged = (state_change_flags) (remote.InternalChanged
                    | CONNECTION_LOST);
...
#define TYPE2NAME(_type)    (type==_type)? _T(#_type)
    const TCHAR* name = TYPE2NAME(IN_BUTTONS) : TYPE2NAME(IN_BUTTONS_ACCEL_IR) :
                            TYPE2NAME(IN_BUTTONS_ACCEL_EXT) :
                            TYPE2NAME(IN_BUTTONS_ACCEL_IR_EXT) :
                            TYPE2NAME(IN_BUTTONS_BALANCE_BOARD) : _T("(unknown??)");
    TRACE(_T("ReportType: %s (%s)"), name, (
            continuous ? _T("continuous") : _T("non-continuous")));
#endif
    ReportType = type;
...
void wiimote::RumbleForAsync(unsigned milliseconds)
{
    ...
    AsyncRumbleThread = (HANDLE) _beginthreadex(NULL, 0, AsyncRumbleThreadfunc,
            this, 0, NULL);
...
int wiimote::ParseAccel(BYTE* buff)
{
    ...
    if (Internal.CalibrationInfo.X0)
    {
        Internal.Acceleration.X =
                ...
                / ((float) Internal.CalibrationInfo.XG
                        - Internal.CalibrationInfo.X0);
        Internal.Acceleration.Y =
                ((float) Internal.Acceleration.RawY - Internal.CalibrationInfo.Y0)
                / ((float) Internal.CalibrationInfo.YG
                        - Internal.CalibrationInfo.Y0);
        Internal.Acceleration.Z =
                ((float) Internal.Acceleration.RawZ - Internal.CalibrationInfo.Z0)
                / ((float) Internal.CalibrationInfo.ZG
                        - Internal.CalibrationInfo.Z0);
    }
    else
    {
        Internal.Acceleration.X = Internal.Acceleration.Y =
...
inline float wiimote::GetBalanceValue(short sensor, short min, short mid,
        short max)
{
    if (max == mid || mid == min)
        return 0;
    ...
    float val =
            (sensor < mid) ? 68.0f * ((float) (sensor - min) / (mid - min)) :
...
int wiimote::ParseExtension(BYTE *buff, unsigned offset)
{
    ...
    case wiimote_state::NUNCHUK:
    {
        ...
        // acceleration
        {
            ...
            if ((raw_x != accel.RawX) || (raw_y != accel.RawY)
                    || (raw_z != accel.RawZ))
        ...
        }
        {
            ...
            if (Internal.Nunchuk.CalibrationInfo.MaxX != 0x00)
                joy.X = ((float) raw_x - calib.MidX)
                / ((float) calib.MaxX - calib.MinX);
            if (calib.MaxY != 0x00)
                joy.Y = ((float) raw_y - calib.MidY)
                / ((float) calib.MaxY - calib.MinY);
    ...
    case wiimote_state::GHWT_DRUMS:
    {
        ...
        if (calib.MaxXL != 0x00)
            joyL.X = (joyL.RawX - calib.MidXL)
            / ((float) calib.MaxXL - calib.MinXL);
        if (calib.MaxYL != 0x00)
            joyL.Y = (joyL.RawY - calib.MidYL)
            / ((float) calib.MaxYL - calib.MinYL);
        if (calib.MaxXR != 0x00)
            joyR.X = (joyR.RawX - calib.MidXR)
            / ((float) calib.MaxXR - calib.MinXR);
        if (calib.MaxYR != 0x00)
            joyR.Y = (joyR.RawY - calib.MidYR)
            / ((float) calib.MaxYR - calib.MinYR);
    ...
    case BALANCE_BOARD:
    {
        ...
        Internal.BalanceBoard.Raw.TopR = (short) ((short) buff[offset + 0] << 8
                | buff[offset + 1]);
        Internal.BalanceBoard.Raw.BottomR = (short) ((short) buff[offset + 2] << 8
                | buff[offset + 3]);
        Internal.BalanceBoard.Raw.TopL = (short) ((short) buff[offset + 4] << 8
                | buff[offset + 5]);
        Internal.BalanceBoard.Raw.BottomL = (short) ((short) buff[offset + 6] << 8
                | buff[offset + 7]);
        ...
        Internal.BalanceBoard.Kg.TopL = GetBalanceValue(
        ...
        Internal.BalanceBoard.Kg.TopR = GetBalanceValue(
        ...
        Internal.BalanceBoard.Kg.BottomL = GetBalanceValue(
        ...
        Internal.BalanceBoard.Kg.BottomR = GetBalanceValue(
        ...
        Internal.BalanceBoard.Kg.Total = Internal.BalanceBoard.Kg.TopL
                + Internal.BalanceBoard.Kg.TopR + Internal.BalanceBoard.Kg.BottomL
...
#define IF_TYPE(id) if(type == id) { \
    ...
    case (REGISTER_BALANCE_CALIBRATION & 0xffff):
    {
        ...
        case BALANCE_BOARD:
        {
            ...
            wiimote_state::balance_board::calibration_info &calib =
                    Internal.BalanceBoard.CalibrationInfo;
...
void wiimote::EnableIR(wiimote_state::ir::mode mode)
{
    ...
    static const BYTE ir_sens1[] =
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x90, 0x00, 0xc0 };
    static const BYTE ir_sens2[] =
    { 0x40, 0x00 };
    ...
    while (remote.Handle != INVALID_HANDLE_VALUE)
    {
        ...
        if(!remote.HIDwriteQueue.empty())
...
#endif
            {
                ...
                {
                    remove_entry: EnterCriticalSection(&remote.HIDwriteQueueLock);
#ifdef USE_DYNAMIC_HIDQUEUE
...
bool wiimote::MuteSpeaker(bool on)
{
    ...
    if (on)
        TRACE(_T("muting speaker." ));
    else
        TRACE(_T("unmuting speaker."));
...
bool wiimote::EnableSpeaker(bool on)
{
    ...
    if (on)
        TRACE(_T("enabling speaker."));
    else
        TRACE(_T("disabling speaker."));
...
unsigned __stdcall wiimote::SampleStreamThreadfunc(void* param)
{
    ...
    { OUT_SPEAKER_DATA, 20 << 3, 0xC3, 0xC3, 0xC3, 0xC3, 0xC3, 0xC3, 0xC3,
        ...
        else
        {
            ...
            if (current_sample)
            {
                if (sample_index < current_sample->length)
                {
                    ...
                    sample_report[1] = (BYTE)(
                            (report_entries << 3) | remote.GetRumbleBit());
#ifdef FIRSTFRAME_IS_SILENT
                    if(silent_frame)
                    {
                        // send all-zeroes
...
#endif
                    {
                        for (unsigned index = 0; index < report_entries; index++)
                            sample_report[2 + index] =
                                    current_sample->samples[(sample_index >> 1)
                                                            + index];
                        remote.WriteReport(sample_report);
...
#define READ_SIZE(ptr,size) if(fread(ptr, size, 1, file) != 1) {           \
    ...
    while (1)
    {
        ...
        if (!strncmp(chunk_header.ckID, "fmt ", 4))
        {
            ...
            int format = extensible ? wf.xe.SubFormat.Data1 : wf.x.wFormatTag;
            ...
            {
                TRACE(_T(".. %d bit, %d channel%s"), wf.x.wBitsPerSample, wf.x.nChannels, (
            ...
            {
                WARN(_T(
                        "Couldn't (loosely) match .wav samplerate %u Hz to speaker"), sample_freq);
                goto unsupported;
            }
        }
        else if (!strncmp(chunk_header.ckID, "data", 4))
        {
            ...
            if (read != total_samples)
            {
                ...
                WARN(_T(
                        "found %s .wav audio data than expected (%u/%u samples)"), (
                                (read < total_samples) ? _T("less") : _T("more")), read, total_samples);
    ...
    corrupt_file: WARN(_T(".wav file is corrupt"));
    fclose(file);
    return false;

    unsupported: WARN(_T(
            ".wav file format not supported (must be mono 16bit PCM)"));
    fclose(file);
    return false;
}
...
bool wiimote::Load16BitMonoSampleRAW(const TCHAR* filepath, bool _signed,
        speaker_freq freq, wiimote_sample &out)
{
...
bool wiimote::Convert16bitMonoSamples(const short* samples, bool _signed,
        DWORD length, speaker_freq freq, wiimote_sample &out)
{
    ...
    static const int diff_table[16] =
    { 1, 3, 5, 7, 9, 11, 13, 15, -1, -3, -5, -7, -9, -11, -13, 15 };
    static const int step_scale[16] =
    { 230, 230, 230, 230, 307, 409, 512, 614, 230, 230, 230, 230, 307, 409,
            512, 614 };
        ...
        speaker_freq freq_override)
{
    ...
    BYTE bytes[9] =
    {   0x00, 0x00, 0x00, 10+freq, vol, 0x00, 0x00, 0x01, 0x01};
...
bool wiimote::StartSampleThread()
{
    ...
    SampleThread = (HANDLE) _beginthreadex(NULL, 0, SampleStreamThreadfunc, this, 0,
            NULL);
...
bool wiimote::PlaySquareWave(speaker_freq freq, BYTE volume)
{
    ...
    BYTE bytes[9] =
    {   0x00, 0x00, 0x00, freq, volume, 0x00, 0x00, 0x01, 0x1};
    WriteData(0x04a20001, sizeof(bytes), bytes);
#else
...
void wiimote::RecordState(state_history &events_out, unsigned max_time_ms,
